...
func _initialize() -> void:
	...
	h.check(Endpoints.ws_error_message("invalid move: rank dispute already pending") == "当前已有排名争夺正在处理", "ws 并发争夺冲突使用中文提示")
	h.check(Endpoints.ws_error_message("invalid move: rank dispute is cooling down") == "刚刚已被拒绝，请稍后再发起争夺", "ws 重复争夺冷却使用中文提示")
	h.check(Endpoints.ws_error_message("invalid move: confirmed player cannot challenge") == "已锁定预测后不能发起争夺", "ws 已锁玩家争夺限制使用中文提示")
