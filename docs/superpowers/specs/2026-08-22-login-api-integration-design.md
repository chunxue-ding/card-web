...
**目标**：把登录页的「登 录」「游客一键登录」和注册页的「注 册」接到真实后端，成功后进入极简大厅占位场景。登录页 ↔ 注册页的场景跳转脚本已存在（`login.gd` / `register.gd`），本次补齐网络行为。

**非目标**：忘记密码、token 持久化自动登录、Web 导出 CORS（首次导出时后端补 CORS 中间件）。
...
| 注册 | POST `/api/v1/auth/register` | `{email, password}` | 同上（注册即登录，`is_new_user=true`） |
...
- 错误码：100001 参数不合法 / 100003 邮箱已注册 / 100004 邮箱或密码错误 / 100005 未设密码 / 100006 尝试过于频繁（游客登录按 IP 限流 10 分钟 5 次；注册/登录按邮箱限流）
...
| `scripts/api/endpoints.gd` | 路径常量；错误码枚举 + 中文消息映射（100001/100003/100004/100006/401/-1） |
| `scripts/api/api_client.gd` | `ApiClient extends Node`：内部 HTTPRequest，`await post_json()/get_json()`；10s 超时 |
| `scripts/api/auth_api.gd` | `AuthApi.login/register/guest_login/logout`，返回 `(user_dict 或 ApiError)` |
| `scripts/session.gd` | autoload `Session`：持有 token/user；device_id 生成与持久化；登录/注册/登出入口 |
| `scenes/login/login.gd` | 输入收集、前置校验、调 Session、错误展示、切场景（新建账号跳注册页已实现） |
| `scenes/register/register.gd` | 注册输入收集、前置校验（含两次密码一致）、调 Session.register、错误展示；「登录账户」返回登录页已实现 |
| `scenes/lobby/lobby.tscn` + `lobby.gd` | 极简大厅：昵称/邮箱或「游客」/余额 + 退出按钮 |

修改：`project.godot`（`[autoload] Session="*res://scripts/session.gd"`）、`login.tscn`（补 signal 连接、表单底部错误 Label）、`register.tscn`（RegisterButton 连接、表单底部错误 Label、AccountInput placeholder 改「邮箱」——后端仅支持邮箱注册，提前对齐输入预期）。
...
**注册**：点「注 册」→ 前置校验（邮箱格式、密码 ≥ 8、两次密码一致）→ `await Session.register(email, pwd)` → `POST /auth/register`；后端注册即登录返回 token，成功直接进大厅；失败显示中文消息（100003 →「该邮箱已注册」）。
...
**UI 反馈**：请求期间登录/注册/游客按钮 disabled + 文本变「登录中…/注册中…」；「忘记密码？」保持现状不接（后端无对应接口）。

## 5. 验收（手动）

前置：`cd /Users/dn/user && make up && make run`。

1. 注册页注册新邮箱 → 注册成功直接进大厅，昵称/余额正确（初始 5000）
2. 重复注册同一邮箱 → 显示「该邮箱已注册」，不切场景；两次密码不一致 → 客户端直接提示
3. 用注册的账号密码登录 → 进大厅；错误密码 → 显示「邮箱或密码错误」，不切场景
4. 游客登录进大厅；退出后再游客登录 → 同一账号（日志确认 `is_new_user=false`）
5. 停掉后端 → 点登录 → 显示「无法连接服务器」
